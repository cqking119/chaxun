...
/**
 * Created by Malu on 2017-07-12.
 */
public class ExportExcel<T> {
    public void exportExcel(String[] headers, Collection<T> dataset, String fileName, HttpServletResponse response) {
        XSSFWorkbook workbook = new XSSFWorkbook();//workbook是表格对象
        // 生成一个表格
        XSSFSheet sheet = workbook.createSheet(fileName);//通过表格对象产生excel表单，注入表格名filename
        // 设置表格默认列宽度为15个字节
        sheet.setDefaultColumnWidth((short) 20);
        // 产生表格标题行
        XSSFRow row = sheet.createRow(0);
        for (short i = 0; i < headers.length; i++) {//产生excel标题的表头
